...
gene.use <- all_genenames
df1 <- data.frame(gene.use,newnames)
rownames(df1) <- df1$gene.use
df2 <- df1[rownames(obj),]
all_genenames <- df2$gene.use
gene.use <- df2$gene.use
newnames <- df2$newnames
...
RNA <- obj@assays[a1][[1]]  
  if (nrow(RNA) == length(newnames)) {
    ...
    if (length(RNA@var.features)) {
        ...
        newnames1 <- df1$v2
        c1 <- which(all_genenames1 %in% RNA@var.features)
        RNA@var.features            <- newnames1[c1]
    }
    if (length(RNA@scale.data)){
        df1 <- order_name(v1=all_genenames,v2=newnames,ref=rownames(RNA@scale.data))
        all_genenames1 <- df1$v1
        newnames1 <- df1$v2
        c1 <- which(all_genenames1 %in% rownames(RNA@scale.data))
        rownames(RNA@scale.data)    <- newnames1[c1]}
...
all_genenames2 <- df1$v1
newnames2 <- df1$v2

c1 <- which(all_genenames2 %in%  rownames(obj))
all_genenames1 <- all_genenames2[c1]
newnames1 <- newnames2[c1]
...
if (length(obj@reductions$pca)){
    ...
    all_genenames1 <- df1$v1
    newnames1 <- df1$v2
    c1 <- which(all_genenames1 %in% hvg)
    rownames(obj@reductions$pca@feature.loadings) <- newnames1[c1]
}
